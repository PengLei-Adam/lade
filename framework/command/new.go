...
var newCommand = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/PengLei-Adam/lade/releases，默认为最新版本)：",
			}
			...
			if version != "" {
				// 确认版本是否正确
				release, _, err = client.Repositories.GetReleaseByTag(context.Background(), "PengLei-Adam", "lade", version)
				if err != nil || release == nil {
					fmt.Println("版本不存在，创建应用失败，请参考 https://github.com/PengLei-Adam/lade/releases")
					return nil
				}
			}
			if version == "" {
				release, _, err = client.Repositories.GetLatestRelease(context.Background(), "PengLei-Adam", "lade")
		...
		fmt.Println("lade框架版本：", release.GetTagName())

		templateFolder := filepath.Join(currentPath, "template-lade-"+version+"-"+cast.ToString(time.Now().Unix()))
		...
		for _, fInfo := range fInfos {
			// 找到解压后的文件夹
			if fInfo.IsDir() && strings.Contains(fInfo.Name(), "lade-") {
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte("module github.com/PengLei-Adam/lade"), []byte("module "+mod))
				c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/PengLei-Adam/lade "+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/PengLei-Adam/lade/app"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/PengLei-Adam/lade/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
