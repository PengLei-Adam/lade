...
// LadeConfig 表示lade框架的配置文件服务
type LadeConfig struct {
	c        framework.Container    // 容器
	folder   string                 // 配置所在文件夹
	keyDelim string                 // 路径分隔符是
...
// loadConfigFile 读取某个配置文件
func (conf *LadeConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// conf内的map中删除文件对应的内容，文件自身不变
func (conf *LadeConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
...
// NewLadeConfig 初始化Config方法
func NewLadeConfig(params ...interface{}) (interface{}, error) {
	...
	for _, file := range files {
		fileName := file.Name()
		err := ladeConf.loadConfigFile(envFolder, fileName)
		if err != nil {
			log.Println(err)
			continue
		}
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						ladeConf.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						ladeConf.removeConfigFile(folder, fileName)
					}
...
// 查找某个路径的配置项，输入path，为Split之后的字符串切片
...
// 通过path获取配置项的通用函数
func (conf *LadeConfig) find(key string) interface{} {
	conf.lock.Lock()
	defer conf.lock.Unlock()
