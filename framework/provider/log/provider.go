...
import (
	...
	"github.com/PengLei-Adam/lade/framework/provider/log/services"
)

// LadeLogServiceProvider 服务提供者
type LadeLogServiceProvider struct {
	framework.ServiceProvider

	Driver string // Driver日志输出方式
...
// Register
func (l *LadeLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewLadeConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewLadeSingleLog
	case "rotate":
		return services.NewLadeRotateLog
	case "console":
		return services.NewLadeConsoleLog
	case "custom":
		return services.NewLadeCustomLog
	default:
		return services.NewLadeConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *LadeLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *LadeLogServiceProvider) IsDefer() bool {
	return false
}

// Name 定义对应的服务字符串凭证
func (l *LadeLogServiceProvider) Name() string {
...
func (l *LadeLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
